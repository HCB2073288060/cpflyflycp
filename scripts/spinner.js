...
class Spinner {
    ...
    spin() {
        ...
        // 计算随机旋转角度（至少5圈，最多9圈）
        const randomSpins = 5 + Math.random() * 4;
        ...
        // 优化：确保转盘总是顺时针旋转足够的圈数
        // 计算目标旋转角度，但确保它不会减少总旋转角度
        const targetRotation = (targetIndex * this.sectorAngle + this.sectorAngle / 2) % 360;
        
        // 总旋转角度 - 确保转盘至少旋转randomSpins圈
        // 通过添加360的倍数来确保旋转方向一致
        const totalRotation = randomRotation + targetRotation;
